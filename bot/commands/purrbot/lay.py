...
class Lay(commands.Cog):
    def __init__(self, bot):
        self.bot = bot

    @commands.command(name="lay", help="Lay down solo or with someone using a random gif 🛏️")
    async def lay(self, ctx: commands.Context, *, arg: str = None):
        ...
        # Get random gif from 'lay' category
        async with aiosqlite.connect(DATABASE) as db:
            cursor = await db.execute("SELECT url FROM media WHERE category = 'lay'")
            results = await cursor.fetchall()

        if not results:
            await ctx.reply("❌ No lay gifs found in database.")
            return

        gif_url = random.choice(results)[0]

        # Use same message for solo or with member
        desc = f"**{ctx.author.display_name}** is laying down "
