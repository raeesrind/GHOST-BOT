...
class Hug(commands.Cog):
    ...
    @commands.command(name="hug", help="Hug someone using a random gif")
    async def hug(self, ctx: commands.Context, *, arg: str = None):
        ...
        if not member:
            await ctx.reply("❌ Please mention someone or reply to their message to hug.")
            return

        # Get random gif from database
        async with aiosqlite.connect(DATABASE) as db:
            cursor = await db.execute("SELECT url FROM media WHERE category = 'hug'")
            results = await cursor.fetchall()

        if not results:
            await ctx.reply("❌ No hug gifs found in database.")
            return

        gif_url = random.choice(results)[0]

        embed = discord.Embed(
            description=f"**{ctx.author.display_name}** Hugs You **{member.display_name}**",
