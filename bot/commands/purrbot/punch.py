...
class Punch(commands.Cog):
    ...
    @commands.command(name="punch", help="Punch someone using a random gif")
    async def punch(self, ctx: commands.Context, *, arg: str = None):
        ...
        if not member:
            await ctx.reply("❌ Please mention someone or reply to their message to punch.")
            return

        # Get random gif from database
        async with aiosqlite.connect(DATABASE) as db:
            cursor = await db.execute("SELECT url FROM media WHERE category = 'punch'")
            results = await cursor.fetchall()

        if not results:
            await ctx.reply("❌ No punch gifs found in database.")
            return

        gif_url = random.choice(results)[0]

        embed = discord.Embed(
            description=f"**{ctx.author.display_name}** Punches You **{member.display_name}**",
