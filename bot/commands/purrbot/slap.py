...
class Slap(commands.Cog):
    ...
    @commands.command(name="slap", help="Slap someone using a random gif")
    async def slap(self, ctx: commands.Context, *, arg: str = None):
        ...
        if not member:
            await ctx.reply("❌ Please mention someone or reply to their message to slap.")
            return

        # Get random gif from database
        async with aiosqlite.connect(DATABASE) as db:
            cursor = await db.execute("SELECT url FROM media WHERE category = 'slap'")
            results = await cursor.fetchall()

        if not results:
            await ctx.reply("❌ No slap gifs found in database.")
            return

        gif_url = random.choice(results)[0]

        embed = discord.Embed(
            description=f"**{ctx.author.display_name}** SLAPS YOU **{member.display_name}**",
