...
class Poke(commands.Cog):
    ...
    @commands.command(name="poke", help="Poke someone using a random gif 👈")
    async def poke(self, ctx: commands.Context, *, arg: str = None):
        ...
        if not member:
            await ctx.reply("❌ Please mention someone or reply to their message to poke.")
            return

        # Fetch random gif from 'poke' category
        async with aiosqlite.connect(DATABASE) as db:
            cursor = await db.execute("SELECT url FROM media WHERE category = 'poke'")
            results = await cursor.fetchall()

        if not results:
            await ctx.reply("❌ No poke gifs found in database.")
            return

        gif_url = random.choice(results)[0]

        embed = discord.Embed(
            description=f"**{ctx.author.display_name}** Pokes You **{member.display_name}** ",
